...
for i in range(y_test.shape[-1]):
    y_hat[i] = svm.predict(X_train[i, :])

train_acc = np.sum(y_hat == y_train)/y_train.shape[-1]

y_hat = np.zeros(y_test.shape)
for i in range(y_test.shape[-1]):
    y_hat[i] = svm.predict(X_test[i, :])

test_acc = np.sum(y_hat == y_test)/y_test.shape[-1]

print("Primal problem solved")
print("Accuracy on train set: ", train_acc*100, "%")
print("Accuracy on train set: ", test_acc*100, "%")
